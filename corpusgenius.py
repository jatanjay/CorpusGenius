...
John W. Miller's excellent lyricsgenius wrapper.
...
# importing necessary ____ modules ? "
...
Hence, calling artist_alums(genius_artist_id=181) since bob dylan's artist_id is 181.
...
alongs with their 'years' (not shown here)
...
"""
    ...
    print(f"{Fore.GREEN}Few more moments please!{Style.RESET_ALL}")
    pages = 1
    ...
    print(
        f"List of all tracks generated. (Final number of unique tracks : {Fore.YELLOW}{len(album_tracks_list)}"
        f"{Style.RESET_ALL})\n"
...
Songs that are repeated will be add'ed to the adjacent cell. (set)
...
And suppose if the two songs appended are completley same, they will be discarded since, it's in a set.
...
# exporting as CSV file that has all the lyrics by artist joined one
# behind another and stored in one single cell.
#
...
#     #   all_lyrics = lyrics_by_song('Beatles_tracks.csv')
